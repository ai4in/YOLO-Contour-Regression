...
class BaseTrainer:
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                with torch.cuda.amp.autocast(self.amp):
                    ...
                    # img_vis=(128*batch['img'][0]).cpu().numpy().astype(np.uint8)
                    # if img_vis.shape[-1]==128:
                    #     print(batch)
                    #     print(batch['img'].shape)
                    ...
                    #     contour = (128*batch['segments'][0]).cpu().numpy().astype(np.int32)
                    #     cv2.fillPoly(img0, [contour[0]], color=255)
                    #     cv2.imwrite('/home/data2/mxl/ultralytics/ultralytics-main/aa/2.jpg', img0)
                                
                    #     print(img_vis.shape)
                    #     img_vis=img_vis.reshape(128,128,3)
                    #     box=(128*batch['bboxes'][0]).cpu().numpy().astype(np.int32)
                    #     xmin=(box[0]-box[2]/2)
                    #     ymin=(box[1]-box[3]/2)
                    #     xmax=(box[0]+box[2]/2)
                    #     ymax=(box[1]+box[3]/2)
    ...
    def check_resume(self):
        ...
        # resume = '/home/data2/mxl/ultralytics/ultralytics-main/runs/detect/train4/weights/last.pt'
        if resume:
            try:

...
class Exporter:
    ...
    def __call__(self, model=None):
        ...
        # import numpy as np
        # f=r'/home/data2/mxl/ultralytics/ultralytics-main/dataset/camera_state/val/normal/abaf1_0_1125.jpg'
        # im=cv2.imread(f,cv2.IMREAD_GRAYSCALE)
        # im=cv2.resize(im, (128, 224), interpolation=cv2.INTER_LINEAR)#shape=(H,W)
        # im = np.expand_dims (im, -1)
        # im=np.tile(im,(1,1,3)).astype(np.float32) / 255.
        # H,W,C=im.shape
        # im=im.reshape(H,W,C)
        # im = np.ascontiguousarray(im.transpose((2, 0, 1)))[np.newaxis,...]# HWC to CHW > contiguous
        # self.mxl=im
    ...
    def export_onnx(self, prefix=colorstr('ONNX:')):
        ...
        import onnxruntime
        import numpy as np 
        # ort_session = onnxruntime.InferenceSession(model_onnx.SerializeToString())
        # print(self.mxl)
        # outputs = ort_session.run(None, {'images': self.mxl.astype(np.float32)})
        # print(outputs)

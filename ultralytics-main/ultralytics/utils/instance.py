...
class Instances:
    ...
    def __init__(self, bboxes, segments=None, keypoints=None, bbox_format='xywh', normalized=True) -> None:
        ...
        if len(segments) > 0:
            ...
            segments = resample_segments(segments,n=360)
            # (N, 1000, 2)
            segments = np.stack(segments, axis=0)
        else:
            segments = np.zeros((0, 360, 2), dtype=np.float32)
        self.segments = segments
        # print('segments0000',segments)
        # if len(segments) > 0:
        #     self.segments = np.array(segments,dtype=np.float32)
        # else:
        #     self.segments = np.zeros((0, 36, 2), dtype=np.float32)
        # print('segments0000',self.segments)

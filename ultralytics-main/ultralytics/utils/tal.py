...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        def find_interval(intervals, number):
            for index, (low, high) in enumerate(intervals):
                if low <= number < high:
                    return index
            return None
        def find_intervals_for_tensor(tensor, intervals): 
            indices = tensor.apply_(lambda x: find_interval(intervals,x)) 
            return indices 
        ...
        print(intervals)
        # 查找数字所在的区间
        index = find_intervals_for_tensor(area.to('cpu'),intervals)+1
        stride_gt = index.unsqueeze(-1).to(gt_bboxes.device)


        # Assigned target
        target_labels, target_bboxes, target_scores,target_strides = self.get_targets(gt_labels, gt_bboxes, target_gt_idx, fg_mask,stride_gt)
        ...
        return target_labels, target_bboxes, target_scores, fg_mask.bool(), target_gt_idx,target_strides
    ...
    def get_targets(self, gt_labels, gt_bboxes, target_gt_idx, fg_mask,stride_gt):
        """
        ...
        target_strides = stride_gt.view(-1, 1)[target_gt_idx]
        ...
        return target_labels, target_bboxes, target_scores,target_strides
...
class polarpaperTaskAlignedAssigner(nn.Module):
    #polarpaper
    """
    A task-aligned assigner for object detection.
